...
{!! $kaigai_html !!}
  ...
  <div class="main_column">
      <form class="form" action="{{ env('REQ_INFO_DIR') }}/thanks" method="post" enctype="multipart/form-data">
      {{-- <form class="form" action="https://rifa.life/refasta_public/info/thanks" method="post" enctype="multipart/form-data"> --}}
        {!! csrf_field() !!}
        <input type="text" name="hp_field" id="hp_field" class="hp-field" autocomplete="off">
            ...
            <tr>
              ...
              <td>
                ...
                <input
                  ...
                  name="mail"
                  id="mail"
                  @if((isset($_GET["reregistration"])) && ($_GET["reregistration"] != ""))
                  value="{{ htmlspecialchars($_GET["reregistration"], ENT_QUOTES, "UTF-8") }}"
                  @else
...
営業時間内になるべく早くご連絡をさせて頂きます。"
    ...
    <div id="ss_box" style="text-align: center; display: block; margin: 0px auto 0px;height: unset;float: none!important;">
      <form action="https://www.login.secomtrust.net/customer/customer/pfw/CertificationPage.do" name="CertificationPageForm" method="post" target="_blank" style="margin: 0px;background: none;box-shadow: none;">
        <input type="image" src="https://rifa.life/refasta_wordpress/wp-content/uploads/2022/03/B0310422-1.gif" name="Sticker" alt="クリックして証明書の内容をご確認ください" oncontextmenu="return false;" style="max-width: unset;text-align: center;margin: 0 auto 0 auto;">
        <input type="hidden" name="Req_ID" value="7388475980">
      </form>
    </div>
    
    <div style="text-align: center; font-size: 11px; margin: 0 auto; padding-bottom: 0px;width: 90%;">このサイトは、セコムトラストシステムズ株式会社のサーバ証明書により実在性が認証されています。<br>
